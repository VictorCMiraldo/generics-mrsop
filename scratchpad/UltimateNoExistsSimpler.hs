...
class UltimateGeneric k (f :: k) | f -> k where
  ...
  to   :: SLoT k ts -> Proxy f -> Apply k f ts -> NS k (Rep f) f ts
  from :: SLoT k ts -> Proxy f -> NS k (Rep f) f ts -> Apply k f ts
...
to' :: (UltimateGeneric k f, SSLoT k ts)
    => Proxy f -> Apply k f ts -> NS k (Rep f) f ts
to' = to sslot

from' :: (UltimateGeneric k f, SSLoT k ts)
      => Proxy f -> NS k (Rep f) f ts -> Apply k f ts
from' = from sslot

instance UltimateGeneric (* -> *) [] where
  type Rep [] = '[ '[ ], '[ Explicit V0, Explicit (Rec :@: V0) ] ]

  to s@(SLoT1 _) _ [] = B0 $ NP0
  to s@(SLoT1 _) p (x : xs) = B1 $ NPE @_ @V0 x $ xs :*: NP0

  from s@(SLoT1 _) _ (B0 NP0) = []
  from s@(SLoT1 _) p (B1 (x :*: xs :*: NP0)) = x : xs
